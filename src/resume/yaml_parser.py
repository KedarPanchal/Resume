...
class YamlParser:
    ...
    def _load_helper(self, source: dict, key: str, value: dict | None) -> dict:
        if value is None:
            return source[key] if key in source else {}

        result = {}
        if not hasattr(value, "include") or value["include"] == "all":
                result = source[key] if key in source else {}
            ...
            for item in value["include"]:
                if isinstance(item, str):
                    ...
                    raise ValueError(f"Expected a string or dict in 'include' for key '{key}', but got {type(item)}.")
        else:
            raise ValueError(f"Expected a list or 'all' for 'include' in key '{key}', but got {type(value['include'])}.")

        if hasattr(value, "exclude"):
